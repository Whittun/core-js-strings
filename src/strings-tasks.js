...
function countVowels(str) {
  ...
  for (let i = 0; i < letters.length; i += 1) {
    for (let j = 0; j < str.length; j += 1) {
      if (str.toLowerCase()[j] === letters[i]) {
        counter += 1;
      }
    }
  }
...
function findLongestWord(sentence) {
  const maxNum = Math.max(...sentence.split(' ').map((elem) => elem.length));

  return sentence.split(' ').filter((elem) => elem.length === maxNum)[0];
}
...
function reverseWords(str) {
  ...
  wordArray.forEach((word, key) => {
    wordArray[key] = word.split('').reverse().join('');
  });
